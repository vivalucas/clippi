...
┌──────────────────────────────────────────┐
│          Rust 核心库 (.a / .dll)           │
│ probe / gpu / task / queue / preview / FFI│
└────────────────────┬─────────────────────┘
                     │ 进程调用
                     ▼
              ┌──────────────┐
...
| 核心库 | Rust 静态库 (.a) | Rust 动态库 (.dll) | 逻辑共用，按平台链接 |
...
│    │    ├── task.rs        # 任务配置与执行
│    │    ├── queue.rs       # 批量队列管理
│    │    ├── preview.rs     # 原生解码失败时生成 JPEG 预览
│    │    ├── ffi.rs         # C ABI 接口与 JSON 契约
...
- 普通用户看不到 ffmpeg 命令；失败项可查看并复制原始错误详情
- 设计语言跟随系统：macOS 遵循 Apple HIG，Windows 遵循 Fluent Design
- Windows 与 macOS 共用相同的信息结构、状态语义和任务规则，不追求像素级相同
- 素材方向校正中的单文件视为只有一条素材的队列；旧工具暂时保留单任务入口
- 媒体预览遵循“所见即所得”，最终输出方向必须与预览一致

## 变更记录

| 日期 | 变更内容 | 原因 |
|------|----------|------|
| 2026-09-05 | 新增统一素材方向校正工作区、即时可取消队列和原生播放失败时的 FFmpeg 预览图兜底 | 支持多格式批量方向标准化 |
| 2025-05-14 | 初始版本，基于项目规划文档填充 | 项目初始化 |


## 2026-09-12 界面重设计同步

前端继续分别使用 SwiftUI/WinUI 3，共享导航结构、文案语义、品牌资源和明暗配色，不共享控件代码。assets/brand 为品牌源资源，界面用透明 mark，应用图标用带底板版本。macOS 播放层封装为 NSViewRepresentable + AVPlayerLayer，控制条位于变换外；Windows MediaPlayerElement 也将播放/进度控件置于变换外。
