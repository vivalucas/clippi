...
## 2026-09-12（前端重设计与综合自查）

**触发原因**：用户要求全面改善入口、风格、设置和 logo，并再次检查新旧 bug 及文档规范。

**修改内容**：SwiftUI/WinUI 统一工具导航、独立设置、主题和品牌；Rust 修复缩放及输出校验；本轮补修预览控制条、导航状态、Windows 裁剪和文件夹选择、导入状态通知及队列失败恢复。新增真实 FFI 与 macOS 模型回归脚本。同步编号文档和三语 README。

**遇到的问题**：发现裁剪起点误重置等真实回归；编号文档落后于工作区；模型测试路径比较误报失败。

**解决方式**：按确认路径修复，隔离测试偏好并标准化路径比较；保留历史记录，增加当前交接与综合自查矩阵。

**验证方式**：Rust fmt/clippy/test/Release、FFI 合成媒体、Swift 模型脚本、macOS Debug/Release 与实际 UI、XML/资源引用检查、diff 检查。

**验证结果**：20 单测、11 FFI、8 模型用例通过；macOS 构建和界面通过。Windows 未编译运行，需后续环境验收。详见 [13 综合自查](13-ui-redesign-review.md)。


## 2026-09-12（v1.2.0 版本推进）

**触发原因**：用户明确要求推进版本、提交 GitHub main、创建新版本 tag。

**修改内容**：统一 Cargo.toml/Cargo.lock、macOS MARKETING_VERSION、Windows csproj/manifest、Inno Setup 安装器为 1.2.0。小版本递增对应工具导航重设计及独立设置等新增功能。

**遇到的问题**：原 Rust/macOS 为 1.1.1，Windows/安装器及最新 tag 为 1.1.2。

**解决方式**：统一发布版本，保留现有 CI 的 tag 触发流程。

**验证方式**：版本一致性、Cargo 锁文件、diff 检查；功能验证沿用紧邻上一轮结果。远程 main/tag 推送后核实引用与 CI 触发状态。

**验证结果**：发布产物仍需等待 GitHub Actions，不将 tag 推送等同于产物构建成功。
