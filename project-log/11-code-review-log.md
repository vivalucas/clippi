...
#### 问题 40：Windows 构建中 `StackPanel.IsEnabled` 不存在

- **类型**：Bug / 编译失败
- **严重程度**：高
- **状态**：已修复，待 `v1.0.8` CI 验证
- **位置**：`windows/Clippi/MainWindow.xaml.cs`
- **描述**：`v1.0.7` Windows Actions 在 publish 阶段失败，WinUI `StackPanel` 不包含 `IsEnabled` 属性。
- **自检结论**：成立。GitHub Actions annotation 明确指向 `MainWindow.xaml.cs` 的四处 `StackPanel.IsEnabled`。
- **修复**：对 panel 改用 `IsHitTestVisible` 控制交互，并用 `Opacity` 表示禁用状态；版本推进到 `v1.0.8` 重新发布。
