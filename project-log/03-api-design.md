...
C ABI 和 JSON 字段保持兼容。Scale 的目标宽高必须为正偶数，实际输出按原比例适配并对齐偶数；WebM 重编码使用 VP9/Opus。任务启动前拒绝已存在输出。批量前端先登记全部 task ID 再回放早到回调；返回 ID 数量不符时取消已返回任务、标记失败、恢复可重试状态，并使旧回调失效。
