...
- トリミング・変換・リサイズ・回転と反転・音声の5つのツール
- 独立した設定ページ：システム／ライト／ダークの外観と、保存される既定の出力先

- 単一・複数メディアファイル、またはフォルダ直下の素材を読み込み
...
- 素材補正：プレビューしながら 90°/180° 回転、左右反転、上下反転を一括処理
- 複数ファイルまたはフォルダ直下を読み込み、個別調整または選択項目へ一括適用
- 一般的な動画コンテナを高品質 MP4 に統一し、SDR は H.264、10-bit/HDR は HEVC を自動選択
- システムプレーヤーが再生できない形式は、同梱 ffmpeg で先頭フレームのプレビューを生成
- 進捗、速度、完了、失敗、キャンセル状態のコールバック
- 出力パスの重複回避、処理前の上書き確認と書き込み権限チェック
- UI ローカライズ：简体中文、English、日本語

> ffmpeg / ffprobe はリリースに同梱され、ユーザー側でのインストールや設定は不要です。
...
リリースビルドには ffmpeg / ffprobe が同梱されるため、ユーザーが ffmpeg を別途インストールする必要はありません。開発環境では `scripts/download_ffmpeg.*` でローカルバイナリを取得できます。Rust コアライブラリは、`CLIPPI_FFMPEG_DIR`、アプリ同梱パス、システム `PATH` の順に検索します。
